...
export function loadNavData() {
  ...
  // Original OurAirports-style files
  ...
  // FAA/NASR-style pending files you added
...
function inferTaxiRouteFromTaxiways({
  ...
  // If there is FAA/NASR taxiway data for the airport, use the actual taxiway names.
  // This is still not perfect gate-to-runway routing because the file has no graph/connectivity,
  // but it is no longer blindly guessing Alpha for every airport.
  ...
  // If a combined airport/runway/taxiway CSV exists, try it.
  ...
  // Airport-specific safe fallbacks
  if (ident === 'TKPK') {
...
function chooseTaxiwayDesignators({
  ...
  // Prefer simple, readable taxiway names first.
  ...
  // Common FAA data has lots of segments like A, A_1, A_2 etc.
  // For voice ATC, we do not want "A underscore 1".
  // Use the base taxiway when possible.
  const preferred = [];
