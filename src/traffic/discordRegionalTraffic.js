...
// Fixes: hard PTT/radio lock, complete pilot->ATC->readback exchanges,
// active-frequency/phase scoped traffic, destination names, regional variety.
...
export function startDiscordTrafficLoop({ guildId, sessionId, speakToGuild, getSession }) {
  ...
  const firstDelay = randomInt(16000, 26000);
...
export function getDiscordTrafficLoopStatus(guildId) {
  ...
  if (!state) return { running: false, aircraft: aircraft.length };
  return {
    running: true,
...
async function runTrafficTick(guildId) {
  ...
  try {
    ...
    if (isRadioBusy(guildId, state.minUserQuietMs)) {
      console.log('[DiscordTraffic] Hard radio lock active. Delaying AI pilot.');
      scheduleNextTrafficTick(state, session, randomInt(5000, 10000));
      return;
    }
    ...
    await waitForClearRadio(guildId, { quietMs: state.minUserQuietMs, pollMs: 500, maxWaitMs: 45000, logPrefix: '[DiscordTraffic]' });
    console.log(`[DiscordTraffic] PILOT: ${tx.pilotText}`);

    setTrafficSpeaking(guildId, true);
    const pilotResult = await state.speakToGuild(guildId, tx.pilotText, 'traffic');
    setTrafficSpeaking(guildId, false);
    ...
    await wait(randomInt(2200, 4200));
    await waitForClearRadio(guildId, { quietMs: 1800, pollMs: 500, maxWaitMs: 45000, logPrefix: '[DiscordTraffic]' });

    console.log(`[DiscordTraffic] ATC: ${tx.atcText}`);
    setAtcSpeaking(guildId, true);
    const atcResult = await state.speakToGuild(guildId, tx.atcText, 'atc');
    setAtcSpeaking(guildId, false);
    if (!atcResult?.ok) console.warn('[DiscordTraffic] ATC traffic audio failed.');

    if (tx.readbackText) {
      await wait(randomInt(3000, 5000));
      await waitForClearRadio(guildId, { quietMs: 1500, pollMs: 500, maxWaitMs: 45000, logPrefix: '[DiscordTraffic]' });
      console.log(`[DiscordTraffic] READBACK: ${tx.readbackText}`);
      setTrafficSpeaking(guildId, true);
      await state.speakToGuild(guildId, tx.readbackText, 'traffic');
      setTrafficSpeaking(guildId, false);
    }
...
function seedAircraftWorld(guildId, session = {}) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    aircraft.push({
      ...
      squawk: randomSquawk(),
      phase: pickInitialPhase(normalizePhase(session.phase || 'preflight')),
      altitude: 0,
...
export function createRegionalTrafficTransmission(session = {}, guildId = null) {
  ...
  if (guildId) {
    ...
    aircraft = pickAircraftForFrequency(world, userPhase, controller);
  }
  ...
  const exchange = buildExchange({ scenario, aircraft, callsign: aircraft.callsign, runway: aircraft.runway || runway, controller, departure: aircraft.origin, arrival: aircraft.destination, fix, altitude, flightRules: aircraft.flightRules, aircraftType: aircraft.aircraftType });

  return {
    type: 'discord_regional_traffic', region, regionName: profile.name, controller,
    aircraftId: aircraft.id, callsign: aircraft.callsign.spokenFull,
    ...
    scenario, phase: aircraft.phase, nextPhase: exchange.nextPhase || nextPhaseFor(aircraft.phase),
    runway: aircraft.runway || runway, departure: aircraft.origin, arrival: aircraft.destination,
    fix, altitude, emotion: exchange.emotion, createdAt: Date.now()
  };
}

function buildExchange(ctx) {
  const { scenario, aircraft, callsign, runway, controller, departure, arrival, fix, altitude, flightRules, aircraftType } = ctx;
  const freq = controller.frequency || '118.70';
  const destName = speakAirport(arrival);
  const depName = speakAirport(departure);

  if (flightRules === 'VFR') return buildVfrExchange({ scenario, aircraft, callsign, runway, depName, aircraftType });

  if (scenario === 'clearance') return {
  ...
  if (scenario === 'enroute') return pick([
    { emotion: 'center-steady', pilotText: `${callsign.spokenFull}, level ${altitude}.`, atcText: `${callsign.spokenFull}, roger. Maintain ${altitude}. Traffic twelve o'clock, opposite direction, five miles.`, readbackText: `Maintain ${altitude}, looking for traffic, ${callsign.spokenFull}.`, nextPhase: 'enroute' },
    { emotion: 'center-direct', pilotText: `${callsign.spokenFull}, request direct ${speakFix(fix)}.`, atcText: `${callsign.spokenFull}, cleared direct ${speakFix(fix)}. Resume own navigation.`, readbackText: `Cleared direct ${speakFix(fix)}, ${callsign.spokenFull}.`, nextPhase: 'enroute' },
    { emotion: 'center-ride', pilotText: `${callsign.spokenFull}, light chop at ${altitude}.`, atcText: `${callsign.spokenFull}, ride report copied, light chop at ${altitude}. Thanks.`, readbackText: `Roger, ${callsign.spokenFull}.`, nextPhase: 'enroute' }
  ]);
  ...
  return { emotion: 'generic', pilotText: `${callsign.spokenFull}, request.`, atcText: `${callsign.spokenFull}, say request.`, readbackText: '', nextPhase: aircraft.phase };
}

function buildVfrExchange({ scenario, callsign, runway, depName, aircraftType }) {
  ...
  if (scenario === 'airborne' || scenario === 'enroute') return {
    emotion: 'vfr-departure', pilotText: `${callsign.spokenFull}, off runway ${speakRunway(runway)}, climbing V F R.`,
    atcText: `${callsign.spokenFull}, radar contact. Maintain V F R. Traffic advisories available on this frequency.`,
    readbackText: `Maintain V F R, ${callsign.spokenFull}.`, nextPhase: 'enroute'
  };
  return {
    emotion: 'vfr-ground',
...
function pickAircraftForFrequency(world, userPhase) {
  const scenario = pickScenario(userPhase);
  const phaseWanted = phaseFromScenario(scenario);
  const candidates = world.filter(a => isAircraftPhaseCompatible(a.phase, phaseWanted, userPhase));
  if (candidates.length) return pick(candidates);
  return world[0] || null;
}

function isAircraftPhaseCompatible(aircraftPhase, wanted, userPhase) {
  if (/ground|taxi|preflight|clearance/.test(userPhase)) return /clearance|taxi|push|holding_short|taxi_in/.test(aircraftPhase);
  if (/tower|takeoff|landing|final/.test(userPhase)) return /holding_short|final|taxi_in/.test(aircraftPhase);
  if (/departure|climb|airborne/.test(userPhase)) return /airborne|enroute/.test(aircraftPhase);
  if (/center|cruise|enroute/.test(userPhase)) return /enroute|airborne/.test(aircraftPhase);
  if (/approach|arrival|descent/.test(userPhase)) return /approach|enroute|final/.test(aircraftPhase);
  return true;
}
...
function advanceTrafficAircraft(guildId, aircraftId, nextPhase) {
  ...
  aircraft.lastInstruction = null;
  aircraft.waitingForReadback = false;
  if (nextPhase === 'enroute' && aircraft.nextFixIndex < (aircraft.route?.length || 0) - 1) aircraft.nextFixIndex += 1;
}
...
function phaseFromScenario(scenario) {
  return ({ clearance: 'clearance', taxi: 'taxi', departure: 'holding_short', airborne: 'airborne', enroute: 'enroute', approach: 'approach', landing: 'final' })[scenario] || 'enroute';
}
...
function pickInitialPhase(userPhase) {
  if (/ground|preflight|clearance/.test(userPhase)) return weightedPick([['clearance', 2], ['taxi', 5], ['holding_short', 1], ['taxi_in', 1]]);
  if (/tower|takeoff/.test(userPhase)) return weightedPick([['holding_short', 5], ['final', 3]]);
  if (/departure|climb|airborne/.test(userPhase)) return weightedPick([['airborne', 6], ['enroute', 2]]);
  if (/center|cruise|enroute/.test(userPhase)) return weightedPick([['enroute', 7], ['airborne', 1], ['approach', 1]]);
  if (/approach|descent|arrival/.test(userPhase)) return weightedPick([['approach', 5], ['final', 2], ['enroute', 1]]);
  return weightedPick([['taxi', 2], ['holding_short', 2], ['enroute', 2], ['approach', 1]]);
}

function buildTrafficRoute(profile) {
  const fixes = profile.fixes || [];
  return [...new Set([pick(fixes), pick(fixes)])].filter(Boolean);
}

export function getTrafficLoopIntervalMs(session = {}) {
  const density = normalizeDensity(session?.trafficDensity ?? process.env.TRAFFIC_DENSITY_DEFAULT ?? 2);
  if (density >= 6) return randomInt(45000, 85000);
  if (density >= 5) return randomInt(52000, 95000);
  if (density >= 4) return randomInt(65000, 110000);
  if (density >= 3) return randomInt(75000, 130000);
  if (density >= 2) return randomInt(95000, 160000);
  return randomInt(140000, 210000);
}
...
function createRegionalCallsign(profile) {
  ...
  if (airline.call === 'N') {
    ...
    return { airline: airline.call, number: n, commercial: false, spokenFull: `${airline.spoken} ${speakDigits(n)}` };
  }
  const number = String(randomInt(102, 899));
  return { airline: airline.call, number, commercial: Boolean(airline.commercial), spokenFull: `${airline.spoken} ${speakDigits(number)}` };
}

function pickAltitudeForAircraft(aircraft, userPhase) {
...
function speakAirport(icao = '') { const ident = String(icao || '').toUpperCase().trim(); return AIRPORT_NAMES[ident] || speakIcao(ident); }
...
function speakFrequency(freq = '') { return String(freq).replace('.', ' decimal ').replace(/\b9\b/g, 'niner'); }
function randomSquawk() { let code = ''; while (code.length < 4) code += String(randomInt(0, 7)); return code; }
...
function pickDifferentAirport(list, notThis) { const clean = (list || []).filter(x => x && x !== notThis); return clean.length ? pick(clean) : (notThis || 'TAPA'); }
